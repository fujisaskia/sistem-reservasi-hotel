...
<div class="max-w-lg mx-auto p-8 bg-white text-xs rounded-lg shadow-lg">
    ...
    <form action="{{ route('receptionist.rooms.update', $room->id) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-4"> 
            <div class="flex space-x-2 w-full">
                ...
                <div class="flex-1">
                    ...
                    <input type="number" id="room_number" name="room_number" class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-yellow-300" value="{{ $room->room_number }}" disabled>
                </div>

                <!-- Tipe Kamar dari table room_types -->
                <div class="flex-1">
                    <label for="room_type_id" class="block text-gray-700 mb-2">Tipe Kamar</label>
                    <input type="text" id="room_type_id" class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-yellow-300" value="{{ $room->roomType->tipe_kamar }}" disabled>
                </div>
            </div>
